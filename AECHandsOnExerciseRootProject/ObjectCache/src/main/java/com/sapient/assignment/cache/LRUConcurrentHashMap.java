...
public class LRUConcurrentHashMap<K, V> extends ConcurrentHashMap<K, V> {
	...
	private final CacheEvictionNotifier<V, EvictionReason> notifier;
	
	private Lock lock;
	
	public LRUConcurrentHashMap(int maxSize, CacheEvictionNotifier<V, EvictionReason> notifier) {
	...
	public V put(K key, V value) {
		...
		if(lruKeyList.size() > maxSize){
			lock.lock();
				...
				remove(k);
				notifier.nitifyUser(value, EvictionReason.SIZE_OVERFLOW);
			lock.unlock();
		}
