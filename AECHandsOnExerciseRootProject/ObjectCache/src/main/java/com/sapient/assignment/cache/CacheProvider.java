...
public class CacheProvider<A, V> implements ObjectFinder<A, V> {
	...
	private final CacheNotifier<Future<V>, EvictionReason> notifier;

	public CacheProvider(ObjectFinder<A, V> finder, int cacheSize, long timeToLive, CacheNotifier<Future<V>, EvictionReason> notifier) {
		this.finder = finder;
	...
	public V findObject(final A arg) throws InterruptedException {
		while (true) {
			...
			if (f == null) {
				...
				if (f == null) {
					f = et;
					notifier.notifyHitOrMissToUser(f, false);
					et.run();
				}
				...
				cache.remove(arg);
				notifier.notifyEvictionToUser(f, EvictionReason.TIME_EXPIRED);
				// re-initiate remote search
				continue;
			}
			try {
				// All HIT come here
				notifier.notifyHitOrMissToUser(f, true);
				return f.get();
	...
	public Map<A, Future<V>> getCachedObjects(){
		return new HashMap<A, Future<V>>(cache);
	}
